...
## Кратко: ответы на главные вопросы

| Вопрос | Суть ответа |
|--------|-------------|
| **Сроки** | Контур «скан → отчёт → каталог → exposures» уже работает. **Довести MVP** (UX расследования, полировка скоринга, стабильность): ориентир **4–6 недель** при **1** сильном fullstack. **Production v1** (регулярные фиды, очередь review taint в UI, базовые алерты): ещё **10–14 недель** при одном человеке календарно часто **~3–4 месяца**; ускоряется вторым разработчиком на интеграциях или UI. |
| **Архитектура и владение** | Один репозиторий: Next.js 14 + Supabase (Postgres, Auth). Контракт данных — **миграции**; поведение движка — **`ENGINE.md`**, пользовательская методология — **`METHODOLOGY.md`**. Отдельного «архитектурного подразделения» не предполагается: решения фиксируются в коде и доках. |
| **Команда** | **Минимум:** 1 инженер fullstack (SQL, API, React). **Комфортно:** +1 на фиды/админку или тяжёлую визуализацию. Юридический/комплаенс-контур **вне** этой оценки — без него продукт остаётся **аналитическим инструментом**, а не регуляторным заверением. |
| **Этапы** | **(1) Уже есть:** каталог в БД, резолвер, импорт, админка, отчёты, watchlist, автозахват exposures. **(2) MVP:** layout «расследование», веса ближе к данным в БД, меньше острых углов. **(3) v1:** **идемпотентные** джобы приёма из внешних источников, `/admin/taint`, опционально мониторинг. |
| **Лучшая линия развития** | Не переписывать стек. Наращивать каталог как источник истины; внешние фиды — с **происхождением строки** и дедупом по `(entity_id, currency, address)`; продвижение из `taint_candidates` — **с человеком в контуре**, без автоматической «вины» кошелька. |
| **Автоматический сбор данных** | **Технически да** для **публичных** наборов (санкционные списки, часть community API) — парсинг, нормализация, загрузка в каталог. Ограничения: **лицензии, rate limits, качество сопоставления сущностей**, постоянное сопровождение — не «недостающая магия». |
| **Реально ли при скромных ресурсах?** | **Да** как внутренний / нишевый инструмент: проверка кошельков, курируемый каталог, версии методологии. **Нет** как полный паритет с крупными коммерческими AML-платформами (глобальное покрытие, юр. позиции, 24/7, сертификации) без роста команды и бюджета. |
